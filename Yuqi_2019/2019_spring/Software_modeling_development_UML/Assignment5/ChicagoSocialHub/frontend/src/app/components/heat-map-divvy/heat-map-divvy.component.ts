...
export class HeatMapDivvyComponent implements OnInit {
  ...
  callAtInterval() {
    ...
    if (this.stationIndex + 1 >= this.stationList.length) {
  ...
  getWithinOneDayData() {
    ...
    //stationDataOneDay = this.stationSevenDays.slice(this.stationSevenDays.length- 134000, this.stationSevenDays.length)
    var dateBegin = new Date();
    for (let i = this.stationOneDay.length - 1; i >= 0; i--) {
      var dateEnd = new Date(this.stationOneDay[i].lastCommunicationTime.valueOf());
      var dateDiff = dateBegin.getTime() - dateEnd.getTime();
      ...
      if (dayDiff <= 1) {
        stationDataOneDay = [this.stationOneDay[i]].concat(stationDataOneDay);
      } else {
        break;
      }
    }
  ...
  onMapLoad(mapInstance: google.maps.Map) {
    ...
    const coords = [];
    this.placesService
      .getEntireCityDivvyStationsInformationOneDay()
  ...
  playSevenDayAnimation() {
    d3.selectAll('#dropdownBasic1').text("7 Days");
    d3.selectAll('#play-button').style("opacity", 0);
    this.stationIndex = 0;
    this.stationList = [];
    const coords = [];
    this.stations = this.stationSevenDays;
    this.getLastSixZeroEightData(coords);
  }
  ...
  playOneDayAnimation() {
    ...
    this.stationList = [];
    const coords = [];
    this.stations = this.getWithinOneDayData();
    this.getLastSixZeroEightData(coords);
  }
